...
Running averages are a simple diagnostic tool for assessing MCMC convergence. The basic principle is that if a Markov chain has converged to its stationary distribution, the running average should eventually stabilize around the true mean of the target distribution.

#### How to Interpret Running Averages

**Signs of convergence:**

- The running average stabilizes (stops trending) after some initial period
- Fluctuations around the stable value decrease as $n$ increases
- The stabilization point is consistent across different portions of the chain

**Signs of non-convergence:**

- Persistent trends or drift in the running average
- Large jumps or sudden changes in the running average
- Different behavior when computed from different starting points

::: {.callout-warning}
## Important Limitations of Running Averages

Running averages have several critical limitations:

1. **False stability**: They can appear stable even when the chain hasn't converged, especially if the chain gets "stuck" in one region
2. **Slow to detect problems**: Due to their cumulative nature, they respond slowly to changes in the chain's behavior
3. **Masking effect**: Early samples heavily influence the running average, potentially masking later convergence issues
4. **No information about mixing**: They don't reveal whether the chain is exploring the full distribution

**Key insight**: Running averages are better at detecting when a chain has **not** converged than confirming when it **has** converged. Always use them alongside other diagnostics.
:::

#### Proper Usage Strategy

To use running averages effectively:

1. **Compare multiple starting points**: Calculate running averages from different points in your chain
2. **Look for consistent stabilization**: The stabilization should occur at similar points regardless of where you start
3. **Check for trend changes**: Sudden changes in trend may indicate the chain is moving between different regions
4. **Use as supporting evidence**: Combine with autocorrelation analysis and visual inspection
...
- If running averages from different starting points converge to similar values, this suggests convergence
- The "Local Average Differences" plot helps identify when the chain behavior stabilizes
- A trend test correlation close to zero in the latter portion indicates the absence of systematic drift
- Use this analysis to support (not replace) your autocorrelation-based burn-in determination

Remember: running averages are most valuable when they show **problems** (trends, instability) rather than when they appear stable. Always combine this diagnostic with autocorrelation analysis for robust convergence assessment.
