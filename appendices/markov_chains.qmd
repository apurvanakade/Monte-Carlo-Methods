...
In the example below we consider a random walk on a **barbell graph**—two complete subgraphs (cliques) connected by a single bridge edge. This graph demonstrates poor mixing properties: a random walk starting in one clique takes a very long time to reach and properly sample the other clique.

The barbell graph has a small spectral gap, leading to slow convergence to the stationary distribution. The stationary distribution should be uniform over all nodes, but as we'll see, even after thousands of steps, the distribution remains highly concentrated in the starting clique.

This example illustrates why burn-in is critical. Without sufficient burn-in, samples will not represent the true stationary distribution, leading to biased estimates.
...
# Create a barbell graph: two complete graphs connected by a bridge
n1 = 8  # size of first clique
n2 = 8  # size of second clique
G = nx.barbell_graph(n1, n2)
mc = GraphMC(G)
N = 5000
T_short = 100  # insufficient burn-in
T_long = 2000  # still might not be enough!

# Visualize the graph
plt.figure(figsize=(10, 4))
pos = nx.spring_layout(G, seed=42)
nx.draw(G, pos, with_labels=True, node_color='lightblue', 
        node_size=300, font_size=8, edgecolors='black')
plt.title("Barbell Graph: Two cliques connected by a bridge")
plt.show()

# Start random walk in the first clique (node 0)
walk = mc.random_walk(0, N)

# Short burn-in (clearly insufficient)
walk1 = walk[T_short:]
node_counts1 = {node: walk1.count(node)/len(walk1) for node in mc.nodes}

# Longer burn-in (still might not be enough)
walk2 = walk[T_long:]
...
expected_freq = 1/len(mc.nodes)

axs[0].bar(range(len(node_counts1)), list(node_counts1.values()), color='steelblue', alpha=0.7)
axs[0].set_xlabel('Node')
axs[0].set_ylabel('Frequency')
axs[0].axhline(expected_freq, color='r', linestyle='--', linewidth=2, label='Expected (uniform)')
axs[0].set_title(f'After Discarding First {T_short} Samples\n(Insufficient Burn-in)')
axs[0].legend()
axs[0].set_ylim([0, max(list(node_counts1.values())) * 1.1])

axs[1].bar(range(len(node_counts2)), list(node_counts2.values()), color='steelblue', alpha=0.7)
axs[1].set_xlabel('Node')
axs[1].set_ylabel('Frequency')
axs[1].axhline(expected_freq, color='r', linestyle='--', linewidth=2, label='Expected (uniform)')
axs[1].set_title(f'After Discarding First {T_long} Samples\n(Still Not Fully Mixed)')
axs[1].legend()
axs[1].set_ylim([0, max(list(node_counts1.values())) * 1.1])

# Add vertical line to separate the two cliques
for ax in axs:
    ax.axvline(n1 - 0.5, color='gray', linestyle=':', linewidth=2, alpha=0.5)
    ax.text(n1/2 - 0.5, ax.get_ylim()[1] * 0.9, 'Clique 1', ha='center', fontsize=10, color='gray')
    ax.text(n1 + n2/2 - 0.5, ax.get_ylim()[1] * 0.9, 'Clique 2', ha='center', fontsize=10, color='gray')

plt.tight_layout()
plt.show()

# Show how long it takes to first reach the second clique
first_visit_clique2 = next((i for i, node in enumerate(walk) if node >= n1), None)
print(f"\nFirst visit to second clique at step: {first_visit_clique2}")
print(f"This represents {first_visit_clique2/N * 100:.1f}% of the total walk length")
```
...
Below are the trace plots for @exm-random-walk-on-graph. We can see that the chain does not look uniform even after 1000 samples. After 5000 samples it is starting to mix better, but still shows concentration in the starting clique. Even with 20000 steps, the chain may still not be fully mixed, demonstrating the severe mixing issues with the barbell graph.


```{python}
#| fold: true
# Generate a much longer walk for the third subplot
walk_extended = mc.random_walk(0, 20000)

# trace plot of walk
fig, axs = plt.subplots(1, 3, figsize=(14, 4))

temp = 1000
axs[0].scatter(range(temp), walk[:temp], s=1, alpha=0.5)
axs[0].set_xlabel('Step')
axs[0].set_ylabel('Node')
axs[0].set_title('Trace Plot of Walk ({} steps)'.format(temp))
axs[0].axhline(n1 - 0.5, color='red', linestyle='--', alpha=0.3, linewidth=1)

temp = len(walk)
axs[1].scatter(range(temp), walk[:temp], s=1, alpha=0.5)
axs[1].set_xlabel('Step')
axs[1].set_ylabel('Node')
axs[1].set_title('Trace Plot of Walk ({} steps)'.format(temp))
axs[1].axhline(n1 - 0.5, color='red', linestyle='--', alpha=0.3, linewidth=1)

temp = len(walk_extended)
axs[2].scatter(range(temp), walk_extended[:temp], s=1, alpha=0.5)
axs[2].set_xlabel('Step')
axs[2].set_ylabel('Node')
axs[2].set_title('Trace Plot of Walk ({} steps)'.format(temp))
axs[2].axhline(n1 - 0.5, color='red', linestyle='--', alpha=0.3, linewidth=1)
