...
The most sophisticated method, which corrects for bias and skewness:

$$
\text{CI}_{1-\alpha} = [T^*_{(\alpha_1)}, T^*_{(\alpha_2)}]
$$
...
The **bias-correction** $\hat{z}_0$ is:
$$
\hat{z}_0 = \Phi^{-1}\left(\frac{\#\{T^{*(b)} < T_n\}}{B}\right)
$$

The **acceleration** $\hat{a}$ accounts for the rate of change of the standard error:
$$
\hat{a} = \frac{\sum_{i=1}^{n} (T_{(i)} - \bar{T}_{(\cdot)})^3}{6[\sum_{i=1}^{n} (T_{(i)} - \bar{T}_{(\cdot)})^2]^{3/2}}
$$

where $T_{(i)}$ is the statistic computed with the $i$-th observation deleted (jackknife values).
