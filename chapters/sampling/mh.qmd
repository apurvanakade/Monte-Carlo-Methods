...
def plot_samples(region, X, sigma, n, ax):
  import matplotlib.pyplot as plt

  plt = plot_polygon(region)
  for i in range(len(X)):
    ax.plot(X[i][0], X[i][1], 'r.', markersize=1, alpha=0.5)
  ...
  ax.plot(X[0][0], X[0][1], 'o', markersize=5, label="start", color="black")
  ax.plot(X[-1][0], X[-1][1], 'gx', markersize=10, label="end")
...
sigmas = [1, 0.5, 0.1, 0.01]

for idx, sigma in enumerate(sigmas):
    np.random.seed(0)
    X = Metropolis_Hastings(n, start.copy(), sigma, search_distribution)
    ax = axs[idx // 2, idx % 2]
    plot_samples(region, X, sigma, n, ax)
    ax.set_aspect('equal')
