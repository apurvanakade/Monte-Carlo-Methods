...
In the above algorithm, we rely on @thm-poisson-exponential to generate $X_i \sim \mathrm{Exp}(\lambda)$, we use inverse transform sampling. However, this is inefficient as taking logarithms repeatedly can lead to floating point errors. It is possible to avoid this by using the following insight.
