...
In the above algorithm, we rely on @thm-poisson-exponential. In order to generate $X_i \sim \mathrm{Exp}(\lambda)$, we use inverse transform sampling. However, this is inefficient as taking logarithms repeatedly can lead to floating point errors. 
