...
1. **Function Visualization (Top Panels)**: The contrast between the smooth $\sin(x)$ and rapidly oscillating $25\sin(25x)$ suggests why integration difficulty varies. The high-frequency function has many more sign changes and local extrema that random sampling must capture.

2. **Error Analysis**: The log-log error plot reveals that while both functions follow the expected $O(1/\sqrt{n})$ convergence rate, the high-frequency function consistently maintains higher absolute errors across all sample sizes.

3. **Statistical Summary**: The numerical results quantify the performance difference:
