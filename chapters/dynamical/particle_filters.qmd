...
The filtering distribution $p(x_t | y_{1:t})$ can be computed recursively using **Bayes' theorem** and the **Chapman-Kolmogorov equation**:
...
::: {.callout-note}
## The Particle Filter Algorithm (Sequential Importance Sampling with Resampling)

**Input**: 

- Initial particles $\{x_0^{(i)}\}_{i=1}^N \sim p(x_0)$ with weights $w_0^{(i)} = 1/N$
- Observations $y_{1:T}$
- Number of particles $N$

**For each time step $t = 1, 2, \ldots, T$:**

### Step 1: Prediction (Propagate Particles)
For each particle $i = 1, \ldots, N$:
$$
x_t^{(i)} \sim p(x_t | x_{t-1}^{(i)})
$$

Sample new particle locations according to the state transition model.

### Step 2: Update (Compute Importance Weights)
For each particle $i = 1, \ldots, N$:
$$
\tilde{w}_t^{(i)} = w_{t-1}^{(i)} \cdot p(y_t | x_t^{(i)})
$$

Compute unnormalized weights by multiplying previous weights with the likelihood of the current observation.
...
### Step 4: Estimate Filtering Distribution
The filtering distribution is approximated as:
$$
p(x_t | y_{1:t}) \approx \sum_{i=1}^N w_t^{(i)} \delta(x_t - x_t^{(i)})
$$
...
$$
\mathbb{E}[f(x_t) | y_{1:t}] \approx \sum_{i=1}^N w_t^{(i)} f(x_t^{(i)})
$$

### Step 5: Resampling (Optional but Recommended)
**Check degeneracy**: Compute effective sample size
$$
N_{\text{eff}} = \frac{1}{\sum_{i=1}^N (w_t^{(i)})^2}
$$

**If** $N_{\text{eff}} < N_{\text{threshold}}$ (e.g., $N/2$):
1. Resample $N$ particles $\{\tilde{x}_t^{(i)}\}_{i=1}^N$ from $\{x_t^{(i)}\}_{i=1}^N$ with probabilities $\{w_t^{(i)}\}_{i=1}^N$
2. Set $x_t^{(i)} = \tilde{x}_t^{(i)}$ and $w_t^{(i)} = 1/N$ for all $i$

**Output**: Particle approximation $\{x_t^{(i)}, w_t^{(i)}\}_{i=1}^N$ of $p(x_t | y_{1:t})$ at each time $t$.
:::
