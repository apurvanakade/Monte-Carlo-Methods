...
export async function initPyodide() {
    ...
    const computationCode = await fetch('metropolis_random_walk_computation.py').then(r => r.text());
    await pyodide.runPythonAsync(computationCode);
    
    // Load visualization module
    const visualizationCode = await fetch('metropolis_random_walk_visualization.py').then(r => r.text());
